...
describe('Function "groupByKey"', () => {
  ...
  test('should group students by "age"', () => {
    ...
    expect(groupedData[22].map((student: { name: string; age: number }) => `${student.name} ${student.age}`))
      .toEqual(['Willie 22', 'Glenn 22']);

    expect(groupedData[26].map((student: { name: string; age: number }) => `${student.name} ${student.age}`))
      .toEqual(['Jessica 26']);
  });
