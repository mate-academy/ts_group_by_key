...
describe('Function "groupByKey"', () => {
  ...
  test('should group students by "age"', () => {
    ...
    expect(
      groupedData[22].map((student) => `${student.name} ${student.age}`),
    ).toEqual(['Willie 22', 'Glenn 22']);

    expect(
      groupedData[26].map((student) => `${student.name} ${student.age}`),
    ).toEqual(['Jessica 26']);
  });
  ...
  test('should group students by "surname"', () => {
    ...
    expect(Object.keys(groupedData)).toEqual([
      'Dorsey',
      'Branscome',
      'Barrera',
      'Paez',
    ...
    expect(groupedData.Branscome[0].surname).toEqual('Branscome');

    expect(groupedData.Gandy[0].surname).toEqual('Gandy');
  });
